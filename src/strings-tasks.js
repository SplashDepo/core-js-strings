...
function removeFirstOccurrences(str, value) {
  ...
  if (valueIdx >= 0)
    return (
      str.slice(0, valueIdx) + str.slice(valueIdx + value.length, str.length)
    );
  return str;
...
function removeLastOccurrences(str, value) {
  const valueIdx = str.lastIndexOf(value);
  if (valueIdx >= 0)
    return (
      str.slice(0, valueIdx) + str.slice(valueIdx + value.length, str.length)
    );
...
function sumOfCodes(str) {
  ...
  const result = [...str].reduce((acc, cur) => {
    return (
      acc +
      (Number.isNaN(str.charCodeAt(str.indexOf(cur)))
        ? 0
        : str.charCodeAt(str.indexOf(cur)))
    );
  }, 0);
